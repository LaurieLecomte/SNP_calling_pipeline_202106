...
This repository features the first version of my SNP calling pipeline used for early pilot testing in my M.Sc on Atlantic Salmon in Louis Bernatchez’s lab (ULaval).

The pipeline calls SNPs in 60 Atlantic Salmon genomes from 2 rivers (La Romaine and Puyjalon), using `bcftools mpileup`.

The output VCF has been used by Clare Venney and al. in the upcoming publication :

Scripts were adapted from previous work done by Clément Rougeux, Claire Mérot and Florent Sylvestre.

For an upgraded version of this pipeline, see . 

## Pipeline Overview
This pipeline features 4 main steps, SNP calling, merging and filtering. 

The SNP calling step uses `bcftools mpileup` on 60 bam files, one for each individual (see Raw data below). For the sake of efficiency, this step has been parallelized by chromosome, e.g. SNPs were called across **all** samples at once on **1 chromosome at the time**, which yields 1 VCF file per chromosome.
...
Bam files for all samples and their index. These can be located somewhere on the server or soft-linked in a folder in the pipeline directory, such as `00_bam_files` or `00_sequences` for instance. If $BAM_PATH is the path to bam files : `for file in $BAM_PATH/*; do ln -s $file ./00_sequences; done`
...
Optional : a list of samples ID and their population (and/or sex) for popgen analysis such as PCA or FST calculation, in `02_infos`. 
...
## Raw Data
Adipose fin tissue has been sampled from 60 wild-born Atlantic Salmon in 2020 by the staff of LARSA (). DNA extractions were performed in september 2020 using Qiagen’s DNeasy Blood and Tissue kit. 
